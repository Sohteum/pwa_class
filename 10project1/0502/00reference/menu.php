<ul><!-- 흰색부분이 ul -->
  <li class="menu1">
    <a href="./greet.php">
      <span>회사소개</span>
      <span class="en">company</span>
    </a>
    <h6>회사소개</h6>
    <ul>
      <li><a class="menu1-1" href="./greet.php">인사말</a></li>
      <li><a class="menu1-2" href="./location.php">오시는길</a></li>
      <li><a class="menu1-3" href="./video.php">홍보영상</a></li>
    </ul>
  </li>
  <li class="menu2">
    <a href="./biz-intro.php">
      <span>사업영역</span>
      <span class="en">business</span>
    </a>
    <h6>사업영역</h6>
    <ul>
      <li><a class="menu2-1" href="./biz-intro.php">사업소개</a></li>
      <li><a class="menu2-2" href="./biz-area.php">사업분야</a></li>
    </ul>
  </li>
  <li class="menu3">
    <a href="./gallery1.php">
      <span>갤러리</span>
      <span class="en">gallery</span>
    </a>
    <h6>갤러리</h6>
    <ul>
      <li><a class="menu3-1" href="./gallery1.php">float</a></li>
      <li><a class="menu3-2" href="./gallery2.php">flex</a></li>
      <li><a class="menu3-3" href="./gallery3.php">grid</a></li>
      <li><a class="menu3-4" href="./gallery4.php">masorny</a></li>
      <li><a class="menu3-5" href="./gallery5.php">isotope</a></li>
    </ul>
  </li>
  <li class="menu4">
    <a href="./contact.php">
      <span>온라인문의</span>
      <span class="en">online</span>
    </a>
    <h6>온라인문의</h6>
    <ul>
      <li><a class="menu4-1" href="./contact.php">온라인문의</a></li>
    </ul>
  </li>
  <li class="menu5">
    <a href="./notice.php">
      <span>커뮤니티</span>
      <span class="en">comunity</span>
    </a>
    <h6>커뮤니티</h6>
    <ul>
      <li><a class="menu5-1" href="./notice.php">공지사항</a></li>
      <li><a class="menu5-2" href="./FAQ.php">FAQ</a></li>
      <li><a class="menu5-3" href="./freeboard.php">자유게시판</a></li>
    </ul>
  </li>
  <li class="member menu6">
    <a href="./signIn.php">
      <span>회원</span>
      <span class="en">member</span>
    </a>
    <h6>회원</h6>
    <ul>
      <li><a menu="menu6-1" href="./signin.php">로그인</a></li>
      <li><a menu="menu6-2" href="./signup.php">회원가입</a></li>
      <li><a menu="menu6-3" href="./member.php">회원정보수정</a></li>
    </ul>
  </li>
  <li class="sns">
    <a href="#">
      <span>소셜</span>
      <span class="en">SNS</span>
    </a>
    <h6>SNS</h6>
    <ul>
      <li><a href="#"><i class="fa-brands fa-facebook"></i></a></li>
      <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
      <li><a href="#"><i class="fa-brands fa-instagram"></i></a></li>
    </ul>
  </li>

</ul>
<!-- 포지션픽스트로 다덮고 흰색이 유엘인데 하나하나가 리스트 -->
<!-- 모든 구조는 동일하게  -->