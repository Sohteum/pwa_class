...
const CompMain = () => {
  ...
  const { _todoArr, todoArr , _setTodoArr} = useContext(AppContext)
  const nav = useNavigate()

  const fnSearchHandler = function (e) {
    e.preventDefault()
    const keyword = document.querySelector('input').value;
    const filterArr = [...todoArr].filter(v=>{ //전개연산자는 새미콜론이 없으면 앞이랑 붙어있다고 생각함
  ...
  return (
    <main>
      ...
      <button onClick={()=>{_setTodoArr(todoArr)}}>전체목록</button>
    </main>
  );
};
