...
//v는 차례에 해당하는 그 애를 말함 한개를 네번반복한거 포이치가 네번하는거라는거지 btn 자체는 하나임 하나한테는 이벤트추가가 가능
...
//값을 세번 가져오는것보다 증감연산을 사용하는것이 편함. 초기화를 해야하는데 초기화를 버튼을 누를때마다 하니까 안에서 0을 만든다. 다시말해 클릭할때마다 0을 만들고싶으면 안쪽에서 선언, 클릭과 상관없이 값이 쌓이는거면 밖에서 만들면 됨
